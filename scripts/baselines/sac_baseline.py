"""SAC baseline using stable-baselines3 on GroceryPricingEnv (continuous actions)."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="SAC baseline")
    ...
    env = make_env(n_skus=args.n_skus, wrapper="continuous", seed=args.seed)

    if args.use_wandb:
        wandb.init(project="dreamprice", group="baselines", name=f"sac-seed{args.seed}")
    ...
    print(f"SAC avg episode reward: {avg_reward:.2f}")
